...
- **macOS:** no extra dependencies (reminder dialogs use built-in AppleScript/AppKit). `timesheet autostart`'s session LaunchAgent also watches for macOS's `com.apple.screenIsUnlocked` distributed notification and runs `timesheet start` again on unlock.
- **Linux (KDE/Ubuntu/etc.):** the reminder prompt uses, in order of preference:
  ...
  `notify-send` (from `libnotify-bin`) is used for the "reminders stopped" notification, and `systemd --user` for `timesheet autostart`. With no chooser available at all, reminders fall back to closing the open session with a STOP at the next interval (one STOP, not one per interval) and `timesheet start` defaults to misc/unspecified instead of prompting. `timesheet autostart`'s session unit also watches logind's `LockedHint` session property over the system D-Bus and runs `timesheet start` again on unlock (desktop environments that integrate with logind, e.g. GNOME, KDE, update this property on lock/unlock); if logind or the system bus is unavailable it simply skips the unlock trigger without affecting the STOP-at-logout guarantee.
...
- **Windows:** no extra dependencies to build or run; PowerShell (built in) drives the reminder chooser and shortcuts. The log lives at `%USERPROFILE%\Documents\timesheet.log`. `timesheet start` with no activity shows a full-screen chooser (PowerShell/WinForms) to pick or enter an activity, same as macOS/Linux. The reminder daemon runs in the background and re-prompts at the configured interval; `timesheet stop`/`timesheet interval` stop it (and any open chooser window) the same way as elsewhere. `timesheet autostart` registers a per-user Scheduled Task ("Timesheet Autostart", the per-user, no-admin-required analog of a macOS LaunchAgent or Linux systemd user unit) with two triggers — at login, and at workstation unlock via Task Scheduler's native `SessionStateChangeTrigger`/`SessionUnlock` (only reachable through a Task Scheduler XML definition, not a `schtasks` flag) — both running `timesheet start`; STOP at logoff/shutdown is recorded by the daemon's console control handler. Unlike macOS (LogoutHook) and Linux (systemd `ExecStop` plus a system-level logout-hook unit), there is no second, more-guaranteed mechanism available without admin rights, so this one path is best-effort — Windows does not guarantee it waits for the handler to finish the way launchd/systemd do. Some managed machines block per-user Scheduled Task creation via Group Policy even without admin rights being required; when `schtasks /Create` fails, `timesheet autostart` falls back to a per-user Startup-folder shortcut instead (login only — Startup-folder shortcuts have no way to express a workstation-unlock trigger). `timesheet edit` falls back to `notepad` (instead of `vi`) when `$EDITOR`/`$VISUAL` are unset. `timesheet help` has no `groff`/`less` to page through, so it renders the man page as plain text through `more` instead. `timesheet install` installs the binary as `timesheet.exe` (see [Install](#install)) and creates a per-user Start Menu shortcut ("Start Timesheet") that runs `timesheet.exe start`, so starting work is also a point-and-click action.
...
| `autostart` | Register `timesheet start` on login **and on session unlock**, and `timesheet stop` on logout/shutdown (macOS: LaunchAgents + logout hook, unlock via a distributed notification; Linux: systemd user units + a system-level logout hook, unlock via a logind D-Bus watch; Windows: a per-user Scheduled Task with login and workstation-unlock triggers, falling back to a login-only Startup-folder shortcut if Group Policy blocks Scheduled Task creation, with STOP at logoff/shutdown handled by the daemon's best-effort console control handler — no admin-required second guarantee exists there). Optional first argument: interval (e.g. `5s`, `3m`) to set reminder interval and start the daemon in this session. Without interval: starts the daemon if needed and shows the current reminder interval. Use `timesheet autostart uninstall` to remove. |
...
| `started`   | Record a work start at a **past time**. Args: `timesheet started <start_time> [activity...]`. See [Time formats](#time-formats).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
- **`timesheet autostart [interval]`** registers `timesheet start` at login **and at session unlock**, and `timesheet stop` at logout/shutdown. On Linux the login unit (`ts-autostart-start.service`) is wanted by and ordered after `graphical-session.target`, so the chooser waits for the desktop session instead of racing it — a unit wanted by `default.target` starts seconds before the compositor exists, and the chooser then has no display to open on. The session unit (`ts-autostart-session.service`) runs `timesheet --session-daemon` as its `ExecStart`, which watches logind's `LockedHint` session property over the system D-Bus and runs `timesheet start` again on the transition to unlocked (desktop environments that integrate with logind — GNOME, KDE, etc. — update this property on lock/unlock); if logind or the system bus is unavailable the daemon falls back to idling, leaving the STOP-at-logout guarantee (systemd's own `ExecStop`) unaffected. On macOS the equivalent `com.ts.autostart.session` LaunchAgent watches for the `com.apple.screenIsUnlocked` distributed notification instead. An optional interval (e.g. `5s`, `3m`) sets the reminder interval and starts the daemon in this session so the reminder appears soon. Without interval: starts the daemon if needed and shows the current reminder interval. Startup skips a new START if the last log entry is a STOP less than 60 seconds old, and if startup finds a non-STOP event more than 5 minutes old it backfills a STOP one reminder interval after that event before recording the new START. On macOS/Linux it also installs a **logout hook** as a second guarantee that STOP is recorded at logout/shutdown: on macOS via `com.apple.loginwindow LogoutHook`, on Linux via a system-level systemd unit (`ts-logout-<uid>.service`) whose `ExecStop` runs `timesheet stop` before `shutdown.target`. Installing the hook needs administrator access, so `timesheet autostart` prints the `sudo` command and offers to run it; if you decline, run the printed command yourself. Once the hook is present, later runs skip it. On Windows, both triggers live on one per-user Scheduled Task ("Timesheet Autostart": a `LogonTrigger` plus a `SessionStateChangeTrigger`/`SessionUnlock`, no admin needed), and there is no second guarantee available without admin rights, so the daemon's console control handler is the only STOP-at-logoff mechanism there. `timesheet autostart uninstall` offers to remove it (via `sudo` on macOS/Linux).
