...
| `install`   | Copy the binary (and on macOS the embedded icon as `ts-icon.svg`) to a directory on PATH. Optional: `ts install [install_dir] [repo_path]`. Works without the source repo on macOS (icon is embedded). On Windows, also creates a "Start Timesheet" Start Menu shortcut that runs `ts start`.                                                                                                                                                                                                                                                                                                              |
...
- **`ts stop`** always stops the reminder daemon and shows a dialog that reminders have been stopped (skipped during logout/shutdown). This happens even when the log itself needs no new entry — after an unanswered reminder, which records its own STOP, `ts stop` would otherwise have nothing to write and would leave the daemon running to prompt again one interval later. A prompt that is still on screen is closed along with the daemon: the daemon runs in its own process group, so it is signalled as a group, taking the chooser down with it. A daemon that `ts stop` could not signal (a stray that no longer owns the PID file) notices the file is gone within half a second and exits instead of prompting again, closing its own prompt if one is up.
